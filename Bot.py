...
RANDOM_NUDGES = [
    ...
    "Эй,а сколько можно ебланить иди делом займись чмо",
...
async def quiz_reminder(context: ContextTypes.DEFAULT_TYPE):
    ...
    # Планируем следующий вопрос через 4-8 часов
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "⏸ *Пауза* — временно отключить напоминания\n\n"
        "Поехали! Что пишешь сегодня?",
        parse_mode="Markdown",
        reply_markup=keyboard,
    )

    # Запускаем случайные напоминания для пользователя
    schedule_random_reminder(context, update.effective_chat.id)
    context.job_queue.run_once(
    quiz_reminder,
    when=random.randint(60, 180) * 60,  # первый вопрос через 1-3 часа
    chat_id=update.effective_chat.id,
    )

async def cmd_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Спросить текущую задачу"""
...
async def cmd_streak(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    task = user.get("current_task", "не задана")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if awaiting:
        ...
        # Если написал что-то просто так
        await update.message.reply_text(
...
async def scheduled_reminder(context: ContextTypes.DEFAULT_TYPE):
    """Срабатывает по расписанию (через job_queue)"""
    chat_id = context.job.chat_id
    data = load_data()
    ...
    for uid, user in data["users"].items():
        if not user.get("active", True):
            continue
        question = random.choice(DAILY_QUESTIONS + CHECKIN_QUESTIONS)
        try:
...
async def random_reminder(context: ContextTypes.DEFAULT_TYPE):
    ...
    # Планируем следующее случайное напоминание
    schedule_random_reminder(context, chat_id)

def schedule_random_reminder(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    """Запланировать следующее случайное напоминание"""
    delay = random.randint(RANDOM_REMINDER_MIN, RANDOM_REMINDER_MAX) * 60  # в секундах
    context.job_queue.run_once(
        random_reminder,
        when=delay,
        chat_id=chat_id,
...
def setup_scheduled_jobs(app: Application):
    """Настройка напоминаний по расписанию"""
    # Эти задачи будут срабатывать для всех активных пользователей
    # Для каждого пользователя настраиваем отдельно при /start
    pass
